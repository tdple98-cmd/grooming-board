...
/**
 * Beyond this many days since a dog's last REAL groom, they're "lapsed" rather than "due" — a
 * cycle technically elapsed long ago, but surfacing them here reads as noise, not an actionable
 * call list. Recency of last visit (not days-overdue-past-cycle) is what actually matters, since
 * that's comparable across services with very different cycle lengths.
 */
const MAX_DAYS_SINCE_LAST_VISIT = 180;

/**
 * Compute dogs due for rebook from Supabase appointment rows (with joined dogs).
 * Rows with status "noshow" must be filtered out by the caller before calling this — a no-show
 * never happened, so it must not reset a dog's rebook cycle or count as a future booking.
...
export function computeDueToRebook(appointments, today) {
  ...
  for (const [dogId, { dog, past, future }] of byDog) {
    ...
    if (daysBetween(lastGroomDate, todayStr) > MAX_DAYS_SINCE_LAST_VISIT) continue;
...
export function dueEntryToBoardDog(entry, visit, photoUrls = {}, photoHistory = []) {
  const dog = entry.dog;
  const visitPhotoPath = visit?.photo_url;
  const history = (photoHistory || []).filter((h) => isPhotoPath(h.path));
  const prevPhotoPath = history[0]?.path || null;
  let lastVisit = null;
  if (visit) {
    lastVisit = {
  ...
  return {
    ...
    prevPhotoPath,
    prevPhotoUrl: prevPhotoPath ? photoUrls[prevPhotoPath] || null : null,
    prevPhotoThumbUrl: null,
    collected: false,
    groomer: "",
    checkedInAt: null,
    depositPaid: true,
