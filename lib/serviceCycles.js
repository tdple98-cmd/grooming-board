...
function envRebookWeeks() {
  // Shared by the client bundle (Vite replaces import.meta.env.* at build time) and Node
  // serverless functions (no import.meta.env there — Vercel exposes the same var via process.env).
  const raw =
    (typeof import.meta !== "undefined" && import.meta.env?.VITE_REBOOK_WEEKS) ??
    (typeof process !== "undefined" ? process.env?.VITE_REBOOK_WEEKS : undefined);
